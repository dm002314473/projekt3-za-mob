...
function updateGameArea() {
    ...
    if (myGameArea.keys.touch) {
        var dx = myGameArea.keys.touchX - myGamePiece.x;
        var dy = myGameArea.keys.touchY - myGamePiece.y;

        var angle = Math.atan2(dy, dx);

        var speed = 2;
        myGamePiece.speed_x = Math.cos(angle) * speed;
        myGamePiece.speed_y = Math.sin(angle) * speed;
    }
